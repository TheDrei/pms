...
<!-- END MODAL -->
    ...
    <p align="left"><button id="equipmentButton" type="button" style="border-radius:4px;" class="btn btn-primary btn-md" data-toggle="modal" data-target="#largeModal"><i class="fa fa-plus"></i>&nbsp; Create Annual Procurement Plan</button></p>
    <div class = "card-body">
    <table id="app-data-table" class="table table-striped table-hover" style="background-color: #FFF;width: 100%; ">
      <thead class="thead-light">
        <tr>
          <th style="width: 2%">#</th>
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    window.deleteRow = function(id) {
      ...
      var table = $('#app-data-table').DataTable({
        ajax: '/test/0',
        'processing': true,
        "serverSide": true,
        'language': {
      ...
      $('#app-data-table tbody').on('click', 'deleterow', function() {
    ...
    var t = $('#app-data-table').DataTable({
      "stateSave": true,
      "processing": true,
      "serverSide": false,
      "paging": true,
      ...
      "columns": [{
        ...
        {
          "data": "procurement_project"
        },
        {
          "data": "pmo"
        },
        {
          "data": "is_early_procurement"
        },
    ...
    $('#app-data-table').on('click', '.row-edit-equipment', function(e) {
    ...
    $('#app-data-table').on('click', '.row-accept-equipment', function(e) {
      let varID = getRowID(e.target);
      acceptEquipment(varID);

    });

    $('#app-data-table').on('click', '.row-revert-equipment', function(e) {
      let varID = getRowID(e.target);
      revertEquipment(varID);

    });

    $('#app-data-table').on('click', '.row-dispose-equipment', function(e) {
      let varID = getRowID(e.target);
      disposeEquipment(varID);

    });

    $('#app-data-table').on('click', '.row-transfer-equipment', function(e) {
      let varID = getRowID(e.target);
      transferEquipment(varID);

    });

    $('#app-data-table').on('click', '.row-transfer-report', function(e) {
      let varID = getRowID(e.target);
      transferReport(varID);

    });

    $('#app-data-table').on('click', '.row-view-history', function(e) {    
      let varID = getRowID(e.target);
    ...
    $('#app-data-table').on('click', '.row-generate-semiexpendable-card', function(e) {
      let varID = getRowID(e.target);
      generateSemiExpendablePropertyCard(varID);
    });


    $('#app-data-table').on('click', '.row-generate-ics', function(e) {
      let varID = getRowID(e.target);
      generateICS(varID);

    });

    $('#app-data-table').on('click', '.row-delete-ics', function(e) {
      let varID = getRowID(e.target);
      deleteICS(varID);

    });

    $('#app-data-table').on('click', '.row-ledger-card', function(e) {
      let varID = getRowID(e.target);
    ...
    $('#app-data-table').on('click', '.display-ppe-details', function(e) {
      let varID = getRowID(e.target);
